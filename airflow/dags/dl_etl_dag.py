            ...
            SELECT
                CASE
                    WHEN row_count > 0
                     THEN 'SELECT 1'
                    WHEN row_count = 0
                    THEN 'ALTER TABLE nessie.raw.sales EXECUTE add_files(location => ''s3://datalake/data/sales_summary_updated.parquet'', format => ''PARQUET'')'
                END AS vars
             from is_empty """
...
with DAG(
    ...
    create_schema_nessie = SQLExecuteQueryOperator(
        task_id='create_nessie_schema',
        conn_id='trino_conn',
        sql="""
            CREATE SCHEMA IF NOT EXISTS nessie.raw
    ...
    create_raw_nessie_sales = SQLExecuteQueryOperator(
        task_id='create_nessie_sales_table',
        conn_id='trino_conn',
        sql="""
            CREATE TABLE IF NOT EXISTS nessie.raw.sales (
